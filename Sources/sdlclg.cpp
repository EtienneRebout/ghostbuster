...
void InitialiserAffichage(string Titre, int Largeur, int Hauteur)
{
	...
	VerifierErreur(!ecran, "InitialiserAffichage: Impossible de charger le mode vid�o");
					   ...
					   int Rouge, int Vert, int Bleu)
{
	...
	// Collage de la surface sur l'�cran
...
{
	VerifierErreur(Image<0 || static_cast<unsigned int>((Image)>Images.size()-1), "Num�ro d'image invalide");
